...
#include <fluent-bit/flb_cobj_ra_key.h>
#include <fluent-bit/record_accessor/flb_ra_parser.h>
#include <monkey/mk_core.h>

#include <ctype.h>

static struct flb_ra_parser *cobj_ra_parse_string(struct flb_cobj_record_accessor *cra,
...
static struct flb_ra_parser *cobj_ra_parse_meta(struct flb_cobj_record_accessor *cra,
                                           flb_sds_t buf, int start, int end)
{
    int len;
    struct flb_ra_parser *rp;
 ...
 * $X()                               => built-in function
 */
static int cobj_ra_parse_buffer(struct flb_cobj_record_accessor *cra, flb_sds_t buf)
{
    int i;
    int n;
    int c;
    ...
    for (i = 0; i < len; i++) {
         ...
         */
        if (i > pre) {
            rp = cobj_ra_parse_string(cra, buf, pre, i);
            if (!rp) {
                return -1;
            }
        ...
        rp = cobj_ra_parse_meta(cra, buf, i, end);
        if (!rp) {
            return -1;
        }
        ...
        if (pre < i) {
            rp_str = cobj_ra_parse_string(cra, buf, pre, i);
            if (!rp_str) {
    ...
    if ((i - 1 > end && pre < i) || i == 1 /*allow single character*/) {
        ...
        rp_str = cobj_ra_parse_string(cra, buf, pre, end);
        if (rp_str) {
            mk_list_add(&rp_str->_head, &cra->list);
        }
    }
...
struct flb_cobj_record_accessor *flb_cobj_ra_create(char *str, int translate_env)
{
    ...
    struct mk_list *head;
    struct flb_ra_parser *rp;
    struct flb_cobj_record_accessor *cra;

    /* temporary copy of 'str' to workaround potential issues literal parsing */
    tmp_str = flb_sds_create(str);
    if (!tmp_str) {
        flb_error("[cobj record accessor] cannot allocate temporary buffer");
        return NULL;
    }
    ...
    if (translate_env == FLB_TRUE) {
        ...
        env = flb_env_create();
        if (!env) {
            flb_error("[cobj record accessor] cannot create environment context");
        ...
        if (!buf) {
            flb_error("[cobj record accessor] cannot translate string");
            flb_env_destroy(env);
            flb_sds_destroy(tmp_str);
            return NULL;
        }
    ...
    /* Allocate context */
    cra = flb_calloc(1, sizeof(struct flb_cobj_record_accessor));
    if (!cra) {
        flb_errno();
        flb_error("[cobj record accessor] cannot create context");
        if (buf) {
            flb_sds_destroy(buf);
        }
        flb_sds_destroy(tmp_str);
     ...
     */
    ret = cobj_ra_parse_buffer(cra, p);
    if (buf) {
        flb_sds_destroy(buf);
    }
    if (ret == -1) {
        flb_cobj_ra_destroy(cra);
        return NULL;
    }
...
{
    flb_sds_t tmp = NULL;
    struct flb_cobj_record_accessor *ret = NULL;

    tmp = flb_cobj_ra_create_str_from_list(str_list);
    if (tmp == NULL) {
        flb_errno();
        return NULL;
    }

    ret = flb_cobj_ra_create(tmp, translate_env);
    flb_sds_destroy(tmp);
...
static flb_sds_t cobj_ra_translate_regex_id(struct flb_ra_parser *rp,
...
static flb_sds_t cobj_ra_translate_tag(struct flb_ra_parser *rp, flb_sds_t buf,
...
static flb_sds_t cobj_ra_translate_tag_part(struct flb_ra_parser *rp, flb_sds_t buf,
                                            char *tag, int tag_len)
{
    int i = 0;
    int id = -1;
...
static flb_sds_t cobj_ra_translate_string(struct flb_ra_parser *rp, flb_sds_t buf)
{
    flb_sds_t tmp;
...
static flb_sds_t cobj_ra_translate_keymap(struct flb_ra_parser *rp, flb_sds_t buf,
                                          struct cfl_object cobj, int *found)
{
    int ret;
    int len;
    char *js;
    ...
    crv = flb_cobj_ra_key_to_value(rp->key->name, cobj, rp->key->subkeys);
    if (!crv) {
        *found = FLB_FALSE;
        return buf;
    }
    ...
    else if (crv->type == FLB_COBJ_RA_STRING) {
        tmp = flb_sds_cat(buf, crv->val.string, flb_sds_len(crv->val.string));
    }
    else if (crv->type == FLB_COBJ_RA_NULL) {
        tmp = flb_sds_cat(buf, "null", 4);
    }

    flb_cobj_ra_key_value_destroy(crv);
    return tmp;
}

/*
 * Translate a cobj record accessor buffer, tag and records are optional
 * parameters.
 *
...
{
    return flb_cobj_ra_translate_check(cra, tag, tag_len, cobj, result, FLB_FALSE);
}
                                      ...
                                      int check)
{
    ...
    if (!buf) {
        flb_error("[cobj record accessor] cannot create outgoing buffer");
        return NULL;
    }
    ...
    mk_list_foreach(head, &cra->list) {
        ...
        if (rp->type == FLB_RA_PARSER_STRING) {
            tmp = cobj_ra_translate_string(rp, buf);
        }
        else if (rp->type == FLB_RA_PARSER_KEYMAP) {
            tmp = cobj_ra_translate_keymap(rp, buf, cobj, &found);
            if (check == FLB_TRUE && found == FLB_FALSE) {
                flb_warn("[cobj record accessor] translation failed, root key=%s", rp->key->name);
                flb_sds_destroy(buf);
                return NULL;
            }
        }
        else if (rp->type == FLB_RA_PARSER_REGEX_ID && result) {
            tmp = cobj_ra_translate_regex_id(rp, result, buf);
        }
        else if (rp->type == FLB_RA_PARSER_TAG && tag) {
            tmp = cobj_ra_translate_tag(rp, buf, tag, tag_len);
        }
        else if (rp->type == FLB_RA_PARSER_TAG_PART && tag) {
            tmp = cobj_ra_translate_tag_part(rp, buf, tag, tag_len);
        }

        /* else if (rp->type == FLB_RA_PARSER_FUNC) { */
        /*     tmp = cobj_ra_translate_func(rp, buf, tag, tag_len); */
        /* } */

        if (!tmp) {
            flb_error("[cobj record accessor] translation failed");
            flb_sds_destroy(buf);
            return NULL;
        }
        if (tmp != buf) {
...
/*
 * If the cobj record accessor rules do not generate content based on a keymap or
                       ...
                       char *str, int len)
{
    ...
    return flb_cobj_ra_key_strcmp(rp->key->name, cobj, rp->key->subkeys,
                                  rp->key->name, flb_sds_len(rp->key->name));
}

/*
 * Check if a regular expression matches a cobj record accessor key in the
 * given cfl_object
 */
                            ...
                            struct flb_regex *regex, struct flb_regex_search *result)
{
    ...
    return flb_cobj_ra_key_regex_match(rp->key->name, cobj, rp->key->subkeys,
                                       regex, result);
}

static struct flb_ra_parser* get_ra_parser(struct flb_cobj_record_accessor *cra)
{
...
/*
 * If 'cobj record accessor' pattern matches an entry in the 'cobj', set the
 ...
 * FLB_FALSE.
 */
int flb_cobj_ra_get_kv_pair(struct flb_cobj_record_accessor *cra,
                            ...
                            cfl_sds_t *out_key, struct cfl_variant **out_val)
{
    ...
    return flb_cobj_ra_key_value_get(rp->key->name, cobj, rp->key->subkeys,
                                                       ...
                                                       struct cfl_object cobj)
{
    ...
    return flb_cobj_ra_key_to_value(rp->key->name, cobj, rp->key->subkeys);
}
 ...
 *  @param cobj  the original cfl_object.
                               ...
                               cfl_sds_t in_key, struct cfl_variant *in_val)
{
    ...
    else if (flb_cobj_ra_get_kv_pair(cra, cobj, &s_key, &o_key, &o_val) != 0) {
    ...
    ret = flb_cobj_ra_key_value_update(rp, &cobj, in_key, in_val);
    if (ret < 0) {
        return -1;
    }
                               ...
                               struct cfl_variant *in_val)
{
    ...
    flb_cobj_ra_get_kv_pair(cra, cobj, &s_key, &o_key, &o_val);
    ...
    ret = flb_cobj_ra_key_value_append(rp, &cobj, in_val);
    if (ret < 0) {
        return -1;
    }
